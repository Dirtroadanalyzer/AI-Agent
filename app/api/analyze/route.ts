...
export async function POST(request: Request) {
  try {
    ...
    if(action==="poll"){
      ...
      if(response.status!=="completed")return Response.json({ok:false,error:`Research job ended with status ${response.status}.`},{status:500});
    ...
    const response=await client.responses.create({model:process.env.OPENAI_MODEL||"gpt-5-mini",background:true,text:{format:{type:"json_schema",name:"property_report",strict:true,schema}},input:`You are the lead analyst. Synthesize these four independent investigations into a rigorous report for APN ${apn}. Preserve exact identifiers and parcel-matched evidence. Resolve contradictions; never upgrade inference to fact. Every blocked portal or missing record must become a precise neededItems request with the official URL, human action, and requested return data. Selected tracks: ${propertyTypes.join(", ")}. Buyer objective: ${objective}. Fire-service input: ${fireService}.\n\n${investigations.map((x:any,i:number)=>`--- INVESTIGATOR ${i+1} (${x.kind}) ---\n${x.notes}\nDIRECT EVIDENCE: ${JSON.stringify(x.directEvidence||{})}`).join("\n")}\n\nThe sections array must contain exactly these ids in order: ${sectionIds.join(", ")}. Sources must be actual consulted URLs. Set generatedAt to current ISO time.`});
