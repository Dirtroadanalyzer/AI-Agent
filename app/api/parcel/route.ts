...
export async function GET(request:Request){
  ...
  const parcel=await resolveMohaveParcel(apn);const [overlays,wells,soils]=parcel.status==="resolved"?await Promise.all([resolveSpatialOverlays(parcel),resolveAdwrWells(parcel),resolveUsdaSoils(parcel)]):[null,null,null];const recorder=parcel.status==="resolved"&&overlays?await resolveRecorderLeads(parcel,overlays):null;
  return Response.json({ok:parcel.status==="resolved",parcel,overlays,wells,soils,recorder},{status:parcel.status==="resolved"?200:404});
}
